import { copyPrompt } from '../tests-shared/clipboard';
import { assertElementFeedback } from '../tests-shared/expected-feedback.ts';
...
test.describe('ordinary HTTP comments', () => {
  ...
  test.beforeEach(async ({ page }) => {
    await page.route('http://fixture.invalid/**', route => route.fulfill({ contentType: 'text/html', path: 'demo/index.html' }));
...
test.describe('screenshot comments', () => {
  ...
  test.describe('classic scrollbar capture', () => {
    ...
    for (const quirks of [false, true]) test(`classic scrollbars preserve crop dimensions and pixel alignment (${quirks ? 'quirks' : 'standards'})`, async ({ page, worker, activate }) => {
      if (quirks) {
        const html = (await readFile('demo/index.html', 'utf8')).replace('<!doctype html>', '');
