...
test('the native side panel records, masks, saves and exports a same-origin journey', async ({ journey }) => {
  ...
  if (savedState?.phase !== 'saved') throw new Error(`Expected the saved journey, not ${savedState?.phase}.`);
  expect(filename).toMatch(/^anmerko-journey-[\w.~-]{1,8}-r\d+\.zip$/);
  expect(filename).toBe(journeyArchiveName(draft.id, savedState.revision));
