...
async function mountReview(page: Page, state: unknown, responses: Record<string, unknown> = {}) {
  await page.goto('http://127.0.0.1:4173');
  await page.setContent('<!doctype html><html><body></body></html>');
  await page.evaluate(({ state, responses }) => {
    ...
    (globalThis as any).chrome = {
      runtime: {
        ...
        sendMessage: async (message: { type: string }) => {
          ...
          return structuredClone(responses[message.type] ?? { ok: true });
        },
        onMessage: {
...
test('a save into full saved-journey storage asks the reader to delete saved journeys', async ({ page }) => {
