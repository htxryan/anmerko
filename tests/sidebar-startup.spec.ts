...
const bundle = buildSync({ entryPoints: ['tests/fixtures/sidebar-runtime-harness.ts'], bundle: true, write: false, format: 'iife', loader: { '.css': 'text' } }).outputFiles[0].text;
...
test('native controls wait for a page snapshot and disconnected settings cannot overwrite page state', async ({ page }) => {
  const nativeBundle = buildSync({ entryPoints: ['tests/fixtures/native-sidebar-harness.ts'], bundle: true, write: false, format: 'iife', loader: { '.css': 'text' } }).outputFiles[0].text;
