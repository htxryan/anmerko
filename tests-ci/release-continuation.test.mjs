...
import { continueRelease } from '../scripts/continue-release.mjs';
...
test('only successful explicit Check and Deploy jobs dispatch resume-only release callbacks', () => {
  ...
  assert.match(check, /permissions:[\s\S]*actions: write[\s\S]*steps:\s*\n\s+- uses: actions\/checkout@v7\.0\.1[\s\S]*actions\/setup-node@v7\.0\.0[\s\S]*node-version: 24[\s\S]*node scripts\/continue-release\.mjs/);
...
test('Release self-continuation requires newly written promotion evidence', () => {
  ...
  assert.match(release, /hashFiles\('artifacts\/release-evidence\.json'\) != ''[\s\S]*node scripts\/continue-release\.mjs/);
