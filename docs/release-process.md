...
Press **Run workflow** on **Release** with the default values. The same action is available locally as `task release`. No source SHA, CI run, signed package, or manual evidence is required as a button input. The workflow still requires exact full Check evidence and verified signing before promotion; automation does not establish native store installation or Firefox Android installation and behavior.
