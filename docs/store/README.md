...
Use [listings.json](listings.json) for existing store identities and [store publishing](distribution.md) for submission steps. Replace the entire screenshot set in every listing and locale when refreshing these images; do not retain old screenshots or add the brochure illustration to a store.
