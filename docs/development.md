...
A journey records the site it starts on and holds no host permissions: `activeTab` covers the starting origin, so there is no optional-permission prompt, no `<all_urls>`, and no runtime consent step. Programmatic injection is the access check — it keeps working across same-site path changes, reloads, and link navigations, and it fails once the tab leaves that origin. A navigation to another origin therefore ends the journey in review with `left-site`; the departure is not recorded as a step, but the initiating click is. `webNavigation` stays a required API permission (no data access) so navigation order remains observable.
