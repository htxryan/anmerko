...
Each journey navigation and tab listener wakes an idle background for its event in every tab, so the background keeps them only while a journey needs them: navigation, owner-tab, window-focus, and tab-removal events while a journey starts or records, and tab removal while a launch tab is pending. The alarm listener stays registered; it fires only for the recording deadline and review expiry alarms that journeys schedule and clear. Same-document navigation listeners are filtered to HTTP(S); commits stay unfiltered so an owner tab that opens a browser page still stops as `protected-page`.
