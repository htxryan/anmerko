...
`tests/chromium/journey-real-extension.spec.ts` runs the live lifecycle headlessly in `npm test`. It loads an unmodified copy of the built `dist/`, which has no host permissions, and records through the toolbar's `activeTab` grant. It covers the native side panel through review, masking, Save, Copy Prompt and the ZIP; the `left-site` boundary; a floating-panel journey reviewed in a journey tab; and the next Record journey reusing that tab. It forces reduced motion so the side panel opens without resizing the page mid-capture. `tests/chromium/journey-real-recording.spec.ts` and `tests/chromium/journey-real-lifecycle.spec.ts` load the same unmodified build: the first covers history stamps, values entered before a route change or page load, and clicks into text fields; the second covers a worker that stops before Start or during recording, Back to a replaced launch link, and Record journey after a save. In Firefox, `npm run test:firefox` covers the launch notice, the reload stop in review, a single-page app journey with a value submitted with Enter, and reuse of the floating panel's journey tab headlessly, but not saving or exporting. Firefox needs no doorhanger because required permissions are granted at temporary install.
