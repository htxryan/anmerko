...
The build target's capability (`__TARGET_JOURNEYS__`) is not a switch; no environment variable or preference changes it. Every surface also runs `journeysAvailable` from `src/journey-feature.ts`: journeys stay off on iPhone and iPad, where Edge installs the Chrome and Edge package, and wherever a journey API is missing. The background then binds no journey listeners, the comment bar keeps its three actions, and `journey.html` explains that journeys are unavailable; comments keep working.
