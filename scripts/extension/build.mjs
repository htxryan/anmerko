...
import { browserTarget, browserManifest, journeyBuild } from './browser-targets.mjs';
import { buildVersion } from './build-version.mjs';

const target = browserTarget();
const { build } = await import('esbuild');
const { outdir } = target;
const journeys = journeyBuild(target);
const define = { __ANMERKO_JOURNEYS__: String(journeys) };
await rm(outdir, { recursive: true, force: true });
await mkdir(outdir, { recursive: true });
await cp('public', outdir, { recursive: true });
const manifest = browserManifest(JSON.parse(await readFile('public/manifest.json', 'utf8')),
  buildVersion(JSON.parse(await readFile('package.json', 'utf8')).version), target, journeys);
