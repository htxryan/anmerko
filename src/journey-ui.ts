...
import { JOURNEY_LIMITATIONS, JOURNEY_LIMITS, type CaptureFailure, type StopReason } from './journey-limits';
...
export function mountJourneyUI(root: HTMLElement, client: JourneyClient): () => void {
  ...
  function stopLimitation(draft: JourneyDraftV1): string | undefined {
    const limitation = draft.stopReason ? stopLimitations[draft.stopReason] : undefined;
  ...
  function announceStop(draft: JourneyDraftV1): HTMLElement | null {
    ...
    const text = reason === 'session-storage-limit' && stopLimitation(draft) ? STORAGE_LOSS_NOTICE
      : reason ? stopNotices[reason] : undefined;
